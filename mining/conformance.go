...
// arcRules is one transition's arcs sorted by what the firing rule says each
// does. petri.Arc carries no read flag: a read arc is an inhibitor arc drawn
// transition->place, enabled only while the place holds at least the weight
// (that is how metapetri encodes metamodel.ReadArc, losslessly), and an
// inhibitor arc drawn place->transition blocks while the place holds at least
// the weight. Neither moves a token.
//
// Before this existed, replay, isEnabled and fireTransitionSilent each walked
// net.Arcs on their own and read every place->transition arc as a consuming
// input and every transition->place arc as an output. An inhibitor arc then
// DEMANDED a token from the place it exists to require empty, and a read arc
// PRODUCED a token into the place it only tests — the two failure modes
// metamodel/firing.go's doc lists as the reason a firing rule copied into
// more than one place is a way to be confidently wrong. Concretely: a log
// generated by replaying a net's own SSA read as non-fitting on every
// transition guarded by an inhibitor (sim.pflow.xyz's vet-clinic replayed
// its own synthetic log at 66%), and a one-inhibitor toy net at 0%.
type arcRules struct {
	inputs   map[string]int // consuming: needs weight, takes weight
	outputs  map[string]int // produces weight
	inhibits map[string]int // blocks while place >= weight; moves nothing
	reads    map[string]int // needs place >= weight; moves nothing
}

func rulesFor(net *petri.PetriNet, transID string) arcRules {
	r := arcRules{
		inputs: map[string]int{}, outputs: map[string]int{},
		inhibits: map[string]int{}, reads: map[string]int{},
	}
	for _, arc := range net.Arcs {
		...
		switch {
		case arc.Target == transID && arc.InhibitTransition:
			r.inhibits[arc.Source] = weight
		case arc.Target == transID:
			r.inputs[arc.Source] = weight
		case arc.Source == transID && arc.InhibitTransition:
			r.reads[arc.Target] = weight
		case arc.Source == transID:
			r.outputs[arc.Target] = weight
		}
	}
	return r
}

// gateShortfall is how far the marking is from satisfying the transition's
// non-consuming gates: tokens a read arc needs and does not find, plus one
// per inhibitor arc whose place is not clear. A gate is a condition rather
// than a token, so a blocked inhibitor counts once however many tokens sit
// in the place — the replay's "missing tokens" is a count of unmet
// requirements, and that is one requirement.
func gateShortfall(r arcRules, marking TokenState) int {
	short := 0
	for placeID, required := range r.reads {
		if available := marking[placeID]; available < required {
			short += required - available
		}
	}
	for placeID, limit := range r.inhibits {
		if marking[placeID] >= limit {
			short++
		}
	}
	return short
...
func fireTransition(net *petri.PetriNet, transID string, marking TokenState) (missing, consumed, produced int) {
	...
	// Gates are checked, never moved: a read arc's tokens stay where they
	// are and an inhibitor's place is not drained to clear it.
	if short := gateShortfall(r, marking); short > 0 {
		missing += short
		enabled = false
	}
	...
	for placeID, weight := range r.outputs {
...
func isEnabled(net *petri.PetriNet, transID string, marking TokenState) bool {
	r := rulesFor(net, transID)
	for placeID, required := range r.inputs {
		if marking[placeID] < required {
			return false
		}
	}
	return gateShortfall(r, marking) == 0
}

// fireTransitionSilent fires a transition without tracking metrics. Gates
// (inhibitor and read arcs) move nothing, the same as in fireTransition.
