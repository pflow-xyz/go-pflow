...
type NPC struct {
	...
	HasKey      bool     `json:"has_key"`  // NPC holds a key needed to unlock doors
}
...
const (
	...
	ActionGiveKey  DialogueAction = "give_key"
)
...
func GetDialogue(npc *NPC) []DialogueNode {
	var baseDialogue []DialogueNode
	switch npc.Type {
	case NPCMerchant:
		baseDialogue = getMerchantDialogue(npc)
	case NPCHealer:
		baseDialogue = getHealerDialogue(npc)
	case NPCQuestGiver:
		baseDialogue = getQuestGiverDialogue(npc)
	case NPCWanderer:
		baseDialogue = getWandererDialogue(npc)
	case NPCSage:
		baseDialogue = getSageDialogue(npc)
	default:
		baseDialogue = getGenericDialogue(npc)
	}

	// Add key dialogue nodes if NPC has a key
	if npc.HasKey {
		keyDialogue := []DialogueNode{
			{
				ID:      "has_key",
				Speaker: npc.Name,
				Text:    "Wait... you look like you could use this. I found a key deeper in the catacombs.",
				Choices: []DialogueChoice{
					{Text: "I'll take the key, thank you.", NextID: "give_key"},
					{Text: "What can you tell me about this place?", NextID: "key_info"},
					{Text: "Not now, farewell.", NextID: "key_decline"},
				},
			},
			{
				ID:      "give_key",
				Speaker: npc.Name,
				Text:    "Here, take it. You'll need it to unlock the doors ahead. Be careful down there.",
			...
			},
			{
				ID:      "key_info",
				Speaker: npc.Name,
				Text:    "The deeper levels are sealed with locked doors. Without a key, you won't get far. Take mine - I have no use for it.",
				Choices: []DialogueChoice{
					{Text: "Alright, I'll take the key.", NextID: "give_key"},
					{Text: "I'll manage without it.", NextID: "key_decline"},
				},
			},
			{
				ID:      "key_decline",
		...
		baseDialogue = append(keyDialogue, baseDialogue...)
	}
