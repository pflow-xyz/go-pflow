...
| Item | State |
|------|-------|
| G1. Promote petri-pilot's Gillespie SSA core (propensity computation, `combinations`, the `ssa` event loop — `pkg/runtime/sim/sim.go`'s first ~1250 lines, not the HTTP/scenario scaffolding in `scenario.go`/`supply.go`/`http.go`) into a new go-pflow package, rebased on `metamodel.Model`/`ArcRef.Kinetic` (already the right input type — no new modeling concepts). petri-pilot then imports it back instead of owning the algorithm, closing the two-simulators gap `firing.go:41` already flags. | Done — `stochastic` (`stochastic.go`, `supply.go`, `schedule.go`, `guard.go`); petri-pilot `pkg/runtime/sim` is a thin wrapper over it. Parity goldens in `stochastic/testdata/` were generated by the pre-move engine and are asserted `==` on every float |
| G2. A uniform "pick your solver" entry point mirroring Petri.jl's dispatch: the same declared net produces either an ODE trajectory (existing `solver`) or a discrete SSA sample path (G1), with no separate conversion step between them. | Done — `stochastic.Solve` / `stochastic.Method` (`stochastic/solve.go`): `""`/`ssa` → `Simulate` (or `SimulateSchedule` under a schedule), `ode` → `Forecast` |
| G3. Consistency gate: SSA's long-run ensemble average must converge to the ODE relaxation's trajectory on the same mass-action net (the law-of-large-numbers relationship between the two is what makes them "the same model," not two independently-drifting implementations) — a fixture-based test in the style this workspace already holds Go↔JS state roots to. | Done — `stochastic/consistency_test.go`: linear chain (exact mean field, closed form), SIR at N=1000 and N=10000 (relative error must shrink), and a dimerisation case that must *disagree* so the precondition stays honest |
| G4. Julia side (tracked in pflow-jl, not here — much smaller lift): pflow-jl already depends on `AlgebraicPetri.jl`, but AlgebraicPetri 0.9.2/0.10.0 ships no jump or SDE generator — its only DiffEq hooks are `vectorfield` and the OrdinaryDiffEq/ModelingToolkit/Catalyst extensions. pflow-jl's own bridge (`src/algebraic.jl`) only wires up `to_ode_problem`. `to_jump_problem` alongside it requires JumpProcesses.jl as a test dependency and ~25 lines of stoichiometry extraction (`AlgebraicPetri.TransitionMatrices` → `JumpProcesses.MassActionJump`); not a capability already present upstream. | Done (pflow-jl `to_jump_problem`) |
| G5. Gradient-based fitting against discrete/stochastic data — the real novel item, not wiring. `mining.FitRateFunctionsFromLog` is a closed-form estimator from event-log timing stats today; nothing threads SSA-simulated or observed discrete sample paths through `learn.MinimizeGradient` the way `SolveWithSensitivities` does for ODE trajectories. | Done (2da63e8) — `stochastic/likelihood.go`: `NegLogLikelihood` is the exact CTMC log-likelihood of one or more observed `DiscretePath`s (event log-propensities minus the piecewise-constant integral of total propensity over the horizon) with a closed-form gradient per rate, since propensity is linear in its own rate and independent of every other one; `FitDiscrete` hands that objective to `learn.MinimizeGradient`. `Options.OnFire` (zero RNG draws, fired after `fired[chosen]++`) and exported `TokenPlaces` are the plumbing that gets an observed sample path out of `ssa()` in the first place. Proves rate recovery from a single sample path (within 15%), tightening across 20 independent paths, and the gradient against finite differences — independent of whether the optimizer converges |
| G6. Chemical Langevin SDE as the third dispatch target, completing Petri.jl's `ODEProblem`/`JumpProblem`/`SDEProblem` trio off one net: `MethodSDE` in the G2 `Solve` entry point, Euler-Maruyama over the same stoichiometry and propensities G1 promotes — drift `S_j·a_j(x)`, diffusion `S_j·sqrt(a_j(x))·dW_j` per transition, so the noise is *intrinsic* firing noise, not a parameter. Distinct from petri-pilot's `petri_sde` (`pkg/mcp/sde.go`), which is geometric-Brownian noise on user-chosen "volatile" places — exogenous uncertainty layered on the ODE, worth keeping as its own thing. Two real problems to solve: negativity near depletion (reflect/clamp at zero, or hand off to SSA when a count is small — the hybrid that would make `checkDivergence`'s "diverged" prose finally accurate), and a three-way consistency gate in the G3 style: SDE ensemble mean → ODE, SDE variance ≈ SSA variance at large N. Julia side: `to_sde_problem` in pflow-jl reusing G4's stoichiometry extraction. | Planned (after G1-G3) |

Related: the first post-`v0.26.0` follow-up is unifying petri-pilot's
`pkg/mcp/stochastic.go` duplicate SSA onto `stochastic.Simulate`. It seeds
differently (master rng → `Int63` subseeds, default 42), uses sample variance
and evaluates no guards, so the switch changes `petri_stochastic`'s numbers for
a given seed and will re-pin `pkg/mcp/stochastic_test.go`.
