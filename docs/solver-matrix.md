...
Every cell was re-derived from the code named beside it, not from memory, on
the `release/*` line of work past the `v0.28.2` tag — the matrix landed on
`release/matrix-fix` and this provenance note corrects it on
`release/prov-fix`; where a cell says "confirmed" a throwaway run was made to
check the behaviour rather than inferring it. No commit hash is pinned here,
on purpose: the first draft named one and was wrong by a commit as soon as
its own change merged, because that change also edited the sources it cites.
The references resolve against whatever tree ships this file, and the guard
described below is what keeps their line numbers honest.
...
Every reference below is repo-relative and mechanically checked:
`docs/solver_matrix_test.go` extracts each one and fails when a `file.go:N`
citation names a missing file or a line range running past the end of it,
when a bare file or directory reference does not exist, or when a cited test
function is defined nowhere in the repository. That test cannot tell whether
a citation still points at the *right* code — it exists because this page
went stale within one commit of being written, and a dangling reference is
the cheapest symptom of that to catch. Bare file and directory references
are written with their directory so the check is unambiguous: unqualified,
kinetic_test.go and stages_test.go each name two different files here.
...
| **`Portable`** | byte-exact path shared with pflow-rs and pflow-xyz, and with pflow-jl on its `algebraic-petri` branch: SplitMix64 → xoshiro256\*\*, explicit `plog` (`stochastic/portable.go`); forwarded to every schedule segment (`stochastic/schedule.go:84`); goldens in `stochastic/testdata/`, `stochastic/portable_parity_test.go` | **ignored** — the solver draws nothing; documented on the field (`stochastic/stochastic.go:145-151`) | selects `portableSampler.normal()` (Marsaglia polar over the same stream), but **no cross-language fixture pins the normal stream**; documented (`stochastic/stochastic.go:57-61`, `stochastic/portable.go:30-37`) |
